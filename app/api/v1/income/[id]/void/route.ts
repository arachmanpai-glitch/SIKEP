...
import { voidIncome } from "@/services/IncomeService";
...
export async function POST(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    const income = await voidIncome(session, id, parsed.data.reason);
    return apiSuccess(income, "Pemasukan berhasil dibatalkan.");
  } catch (error) {
